...
double get_diversity(vector<Equation> &little_league, vector<Equation> &minor_league, vector<Equation> major_league);

int main(int argc, const char * argv[]) {
    // insert code here...
    srand( static_cast<unsigned int>(time(0)));
    std::cout << "Finding equation...\n";
    ifstream ifs("/Users/albertgo/Documents/MECS_4510/PSet_2/custom.txt");
...
//        cout << major_league.size() << endl;
        ...
        if (evaluations % 25 == 0){
            ...
            double diversity = get_diversity(population, minor_league, major_league);
            cout << "Diversity update: ";
            cout << diversity << endl;
...
double get_diversity(vector<Equation> &little_league, vector<Equation> &minor_league, vector<Equation> major_league){
    ...
    for (int i=0; i<little_league.size(); i++){
        total_error += little_league[i].error;
        all_errors.push_back(little_league[i].error);
        if (i < minor_league.size() && minor_league.size() > 0){
            total_error += minor_league[i].error;
            all_errors.push_back(minor_league[i].error);
        }
        if (i < major_league.size() && major_league.size() > 0){
            total_error += major_league[i].error;
            all_errors.push_back(major_league[i].error);
        }
    }
    double mean = total_error/all_errors.size();
    double variance = 0;
    
    for (int j=0; j<all_errors.size(); j++){
        variance += pow(all_errors[j]-mean, 2);
    }
    std_dev = sqrt(variance/all_errors.size());
